...
ctx8_sa, ctx8_sape, ctx8_pe, ctx8_mlp, ctx8_gas = (    
    with_varients(ctx8, model_name = TextSeqClassifier.__name__, 
        suffix="自注意力机制", 
...
# 数据版本控制变体 (GC)
ctx_gc = with_varients(ctx8, model_name = TextSeqClassifier.__name__,  
        suffix="自注意力机制", 
...
ctx2 = ExpContext(
    description = f"MalAnalysis 二分类实验: {TextSeqClassifier.__name__} + No Plugin", 
    network_config = nc2,
    data_config = dc2, 
    train_config = tc, # 复用 batch_size=512
    adv_config = AdvConfig(
      enable=False
    ),
)
...
ctx2_sa, ctx2_sape, ctx2_pe, ctx2_mlp, ctx2_gas = (    
    with_varients(ctx2, model_name = TextSeqClassifier.__name__, 
        suffix="自注意力机制", 
        plugin=PluginType.SA), 
    with_varients(ctx2, model_name = TextSeqClassifier.__name__, 
        suffix="自注意力机制 + 位置编码", 
...
ctxm = ExpContext(
    description = f"MalDynamic 多标签分类实验: {TextSeqClassifier.__name__} + No Plugin", 
    network_config = ncm,
    data_config = dcm, 
    train_config = tcm, 
    adv_config = AdvConfig(
...
ctxm_sa, ctxm_sape, ctxm_pe, ctxm_mlp, ctxm_gas = (    
    with_varients(ctxm, model_name = TextSeqClassifier.__name__, 
        suffix="自注意力机制", 
        plugin=PluginType.SA), 
    with_varients(ctxm, model_name = TextSeqClassifier.__name__, 
        suffix="自注意力机制 + 位置编码", 
