...
def build_pluggable_block(block_key: PluginType, embed_dim: int) -> nn.Module:
    ...
    """
    from src.models.pluggable.noises import(
        GaussNoiseLinf,
    )
    ...
    if BLOCK_REGISTRY.get(block_key) is None:
        logger.warning(f"Block type {block_key} not recognized, faulting to Identity.")
        return nn.Identity()
    
    return BLOCK_REGISTRY[block_key](embed_dim=embed_dim)